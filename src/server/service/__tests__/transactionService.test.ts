...
describe('TransactionService', () => {
  ...
  describe('addTransaction', () => {
    ...
    it('应该成功添加存款交易并更新账户余额', async () => {
      ...
      expect(accountFundRepository.updateBalance).toHaveBeenCalledWith(1, 200000);
  ...
  describe('getAccountBalance', () => {
    ...
    it('应该支持 beforeTransactionId 参数', async () => {
      vi.mocked(accountFundRepository.findByAccountId).mockResolvedValue(mockAccountFund);
      vi.mocked(transactionRepository.findBeforeTransactionId).mockResolvedValue([]);

      const result = await transactionService.getAccountBalance('1', 123);

      expect(transactionRepository.findBeforeTransactionId).toHaveBeenCalledWith(1, 123);
