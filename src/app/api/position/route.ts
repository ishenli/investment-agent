...
class PositionHttpController extends BaseController {
  static controller = new PositionBizController();
  
  @WithRequestContext()
  static async GET(request: Request) {
    const json = await super.getQuery(request);
    return Response.json(await PositionHttpController.controller.getPositions(json));
  }

  @WithRequestContext()
  static async PUT(request: Request) {
    const body = await super.getBody(request);
    return Response.json(await PositionHttpController.controller.updatePosition(body));
  }
}

export const GET = PositionHttpController.GET;
export const PUT = PositionHttpController.PUT;