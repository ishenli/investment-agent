...
class SearchController extends BaseController {
  ...
  static async GET(request: Request) {
    try {
      ...
      const marketInfoResults = await db
        .select({
      ...
      const companyInfoResults = await db
        .select({
      ...
      const positionResults = await db
        ...
        .from(assetPositions)
        .where(like(assetPositions.symbol, `%${query}%`))
        .limit(pageSize)
        .offset(offset);
      ...
      const transactionResults = await db
        ...
        .from(transactions)
        .where(
          or(like(transactions.symbol, `%${query}%`), like(transactions.description, `%${query}%`)),
        )
        .limit(pageSize)
        .offset(offset);
