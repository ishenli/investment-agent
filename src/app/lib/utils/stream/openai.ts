import { ChatMessageError, CitationItem } from '@typings/message';
...
export const transformOpenAIStream = (
  ...
  if (FIRST_CHUNK_ERROR_KEY in chunk) {
    ...
    const errorData = {
      body: chunk,
      type: 'errorType' in chunk ? chunk.errorType : 'ProviderBizError',
    } as ChatMessageError;
  ...
  try {
    ...
    if (item.delta) {
      ...
      if (typeof content === 'string') {
        ...
        // 判断是否有 <think> 或 </think> 标签
        const hasOpenThinkTag = content.includes('<think>');
        const hasCloseThinkTag = content.includes('</think>');

        // 当前块是否应作为 reasoning 返回：
        // - 之前已处于 thinking 状态
        // - 本块包含开启标签
        // - 本块仅包含闭合标签，但在移除标签后仍有非空内容（如 "理解。\n</think>"）
        const isReasoningNow =
          !!streamContext?.thinkingInContent ||
          hasOpenThinkTag ||
          (hasCloseThinkTag && thinkingContent.trim().length > 0);

        // 更新全局 thinking 状态（在计算 isReasoningNow 之后再更新）
        if (hasOpenThinkTag) {
          streamContext.thinkingInContent = true;
        } else if (hasCloseThinkTag) {
          ...
          if (citations) {
            ...
            return [
              ...
              },
              {
                ...
                id: chunk.id,
                type: isReasoningNow ? 'reasoning' : 'text',
              },
            ];
          }
        }
