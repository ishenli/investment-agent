...
import { SessionCreateConfig } from '@renderer/const/session';
...
import { DB_Session, DB_SessionSchema } from '@renderer/database/schemas/session';
...
class _SessionModel extends BaseModel {
  ...
  async findBySlug(slug: string): Promise<DBModel<DB_Session>> {
    return (await this.table.where('slug').equals(slug).toArray()).at(0);
  }
  ...
  async update(id: string, data: Partial<DB_Session>) {
    data.updatedAt = this.dateToNumber(data.updatedAt);
   ...
   * @param defaultAgentConfig 默认Agent配置
   * @returns 创建或更新的会话
   */
  async createSessionByConfig(
    sessionConfig: SessionCreateConfig,
    defaultAgentConfig: PartialDeep<LobeAgentConfig>,
  ) {
    // 检查是否已存在相同 slug 的会话
    const existingSession = await this.findBySlug(sessionConfig.slug);

    if (existingSession) {
      return;
    }

    // 构建会话数据，合并默认配置和传入的默认配置
    const sessionData: any = {
      config: merge(DEFAULT_AGENT_LOBE_SESSION.config, defaultAgentConfig),
      ...
      meta: {
        ...
        backgroundColor: sessionConfig.backgroundColor,
      },
    };

    // 根据配置决定是否包含创建时间
    if (sessionConfig.includeCreatedAt) {
      sessionData.createdAt = new Date();
    }

    // 创建新的会话
    return await this.create('agent', sessionData);
  }
  ...
  async updateSessionByConfig(
    ...
    // 构建更新数据
    const updateData: Partial<DB_Session> = {
      agentId: sessionConfig.agentId,
      // 只更新默认配置部分，保留用户自定义的配置
      config: merge(existingSession.config, defaultAgentConfig),
      meta: {
        title: sessionConfig.title,
        description: sessionConfig.description,
        avatar: sessionConfig.avatar,
