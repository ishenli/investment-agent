...
import { DB_SessionGroup, DB_SessionGroupSchema } from '@renderer/database/schemas/sessionGroup';
import { SessionGroups } from '@typings/session';
...
class _SessionGroupModel extends BaseModel {
  ...
  async delete(id: string, removeGroupItem: boolean = false) {
    // Update all sessions associated with the sessionGroup to default
    const sessionsToUpdate = await this.db.sessions.where('group').equals(id).toArray();
