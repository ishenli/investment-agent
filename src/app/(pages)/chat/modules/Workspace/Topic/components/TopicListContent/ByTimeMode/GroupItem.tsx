...
const TopicGroupItem = memo<Omit<GroupedTopic, 'children'>>(({ id, title }) => {
  ...
  // 定义有效的时间分组键
  const validTimeKeys = ['month', 'today', 'week', 'yesterday'] as const;
  type TimeKey = typeof validTimeKeys[number];
  
  const timeTitle =
    preformat(id) ??
    (validTimeKeys.includes(id as TimeKey) 
      ? t(`groupTitle.byTime.${id as TimeKey}`)
      : id);
