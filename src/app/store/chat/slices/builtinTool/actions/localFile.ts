...
} from '@/app/tools/local-system/type';
import { localFileService } from '@/app/services/localFileService';
...
> = (set, get) => ({
  ...
  reSearchLocalFiles: async (id, params) => {
    ...
    // await get().updatePluginArguments(id, params); // updatePluginArguments does not exist
  ...
  writeLocalFile: async (id, params) => {
    return get().internal_triggerLocalFileToolCalling(id, async () => {
      ...
      if (result.success) {
        ...
        // const errorMessage = result.error; // result.error does not exist
