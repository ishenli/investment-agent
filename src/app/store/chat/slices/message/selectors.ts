...
const getMeta = (message: ChatMessage) => {
  switch (message.role) {
    case 'user': {
      return {
        avatar: userAvatar(useUserStore.getState()) || DEFAULT_USER_AVATAR,
      };
    }
...
export const getMessageById2 = (messages: ChatMessage[], id: string) =>
  messages.find((m) => m.id === id);

const getMessageById = (id: string) => (s: ChatStoreState) =>
  getMessageById2(activeBaseChats(s), id);
...
const isAIGenerating = (s: ChatStoreState) => {
  // console.warn('isAIGenerating', s.chatLoadingIds, mainDisplayChatIDs(s));
  return s.chatLoadingIds.some((id) => {
    return mainDisplayChatIDs(s).includes(id);
  });
};
