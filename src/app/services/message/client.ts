...
import { FileModel } from '@renderer/database/models/file';
import { MessageModel } from '@renderer/database/models/message';
import { DB_Message } from '@renderer/database/schemas/message';
import {
  ChatFileItem,
  ChatMessage,
  ChatMessageError,
...
export class ClientService implements IMessageService {
  ...
  async getMessages(sessionId: string, topicId?: string): Promise<ChatMessage[]> {
    // console.warn(sessionId, topicId);
  ...
  async updateMessagePluginError(id: string, value: ChatMessagePluginError) {
    throw new Error('Method not implemented.');
  }

  async getMessageLikeStatus(id: string) {
    const message = await MessageModel.findById(id);
    return message?.userLikeTag;
  }
