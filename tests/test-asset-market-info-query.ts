...
async function testAssetMarketInfoQuery() {
  ...
  try {
    ...
    if (marketInfoBySymbol && marketInfoBySymbol.assetMetaIds.length > 0) {
      ...
      const marketInfoByAssetMetaId =
        await assetMarketInfoService.getLatestAssetMarketInfoByAssetMetaId(firstAssetMetaId);
    ...
    const dateRangeMarketInfos = await assetMarketInfoService.getAssetMarketInfosByDateRange(
      startDate,
      endDate,
      5,
    );
