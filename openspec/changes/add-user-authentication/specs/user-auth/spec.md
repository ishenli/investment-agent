...
### Requirement: AuthService Token Validation
此需求 MUST 修改自现有的 AuthService 实现，支持真实的 JWT Token 验证。

#### Scenario: 从 Request 或 LocalStorage 获取 Token
- **GIVEN** 传入 HTTP Request 对象
- **WHEN** AuthService.getCurrentUserId() 被调用
- **THEN** 系统 MUST 首先尝试从 Authorization Header 获取 Token
- **THEN** 如果 Header 没有 Token，系统 MUST 尝试从请求体获取
- **THEN** 如果以上都没有，系统 MUST 从 LocalStorage 读取（客户端环境）

#### Scenario: Token 验证失败
- **GIVEN** Token 格式错误或签名无效
- **WHEN** AuthService.verifyToken() 被调用
- **THEN** 系统 MUST 返回 null
- **THEN** 系统 MUST 记录安全警告日志
- **THEN** 请求处理者 MUST 返回 401 unauthorized 错误

#### Scenario: 开发模式绕过认证
- **GIVEN** 环境变量 `DEV_MODE=true`
- **WHEN** AuthService.getCurrentUserId() 被调用且 Token 验证失败
- **THEN** 系统 MAY 返回默认用户 ID '1'
- **THEN** 系统 MUST 记录警告日志说明使用开发模式

---

## RENAMED Requirements

- FROM: `### Requirement: First Login Detection` (account-management)
- TO: `### Requirement: Auth and Account Initialization` (user-auth)