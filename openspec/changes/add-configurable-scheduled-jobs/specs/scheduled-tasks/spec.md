# scheduled-tasks Specification — Delta

> **Change**: `add-configurable-scheduled-jobs`  
> **Base Spec**: `openspec/specs/scheduled-tasks/spec.md`

本 Delta 在现有 scheduled-tasks capability 的基础上扩展通用可配置定时任务系统。原 spec 中关于 `daily_snapshot`、`price_sync`、`scheduledTaskLogs` 的硬编码逻辑保持不变，新增用户对 AI 洞察和报告类任务的配置管理能力。本次不把 `daily_snapshot` 和 `price_sync` 迁入可配置任务，避免与现有 `useScheduler` 重复执行。
