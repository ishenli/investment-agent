...
- **交互层**：前端自动化规则设置页 + Agent 工具 + IPC 热重载通道

产品定位上，本变更不新增一个与 AI 洞察、AI 报告并列的内容消费中心，而是补齐“自动化触发器”。三者边界为：
- **AI 洞察**：轻量分析流，回答“现在有什么值得注意”
- **AI 报告**：结构化长文沉淀，回答“这段时间表现如何”
- **定时任务**：自动化规则中心，回答“什么时候自动帮我做”
...
- 前端页面放在 `/setting/scheduled-jobs/`（与现有设置路由一致），页面定位为自动化规则中心，仅展示任务配置、启停、执行日志和结果跳转，不展示完整洞察内容或报告正文
...
│   ├── 今日洞察
│   ├── 历史洞察
│   └── 自动分析设置入口 -> /setting/scheduled-jobs?template=insight
├── 报告
│   ├── 周报/月报列表
│   ├── 手动生成
│   └── 定期生成设置入口 -> /setting/scheduled-jobs?template=report_weekly
设置 / 自动化
└── 定时任务
    ├── 自动化规则
    ├── 执行日志
...
交互原则：
- 定时任务页使用“每日 AI 投资洞察”“每周投资复盘”“每月资产报告”等用户任务模板作为主要创建入口，避免在主流程直接暴露 `insight`、`report_weekly`、`report_monthly` 等工程枚举。
- 创建面板采用“模板化任务 + 自然语言说明”的方式：任务类型仍限定为当前支持的三类模板，但用户可以填写“让 AI 帮你做什么”，该说明写入 `config.instructions` 作为 AI 生成偏好。
- 通知方式在创建面板中作为显式字段展示，MVP 仅启用 App 提醒并写入 `config.notificationChannel = "app"`；后续可扩展到邮件、飞书、微信等渠道。
- 定时任务执行结果只展示摘要。洞察任务成功后跳转到 AI 洞察历史；报告任务提交成功或完成后跳转到报告列表/报告详情。
- AI 洞察页和 AI 报告页可提供“自动分析设置”“定期生成设置”入口，但不各自实现一套独立调度配置表单，避免配置能力重复。
- “立即分析/生成报告”仍保留在内容页；“启用、禁用、修改周期、查看执行日志”统一收敛到定时任务页。
...
| 优先级 | 用户故事 | 独立验证 |
|--------|---------|---------|
| P1 | 用户可以在自动化规则中心配置定时任务（如每周一 9 点生成周报），任务在 Electron 主进程中按时触发 | 配置后等待到 cron 时间点，检查系统通知是否弹出，数据库日志是否有记录；结果内容回到洞察/报告页消费 |
