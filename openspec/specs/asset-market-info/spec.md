# asset-market-info Specification

## Purpose
管理资产相关的市场信息，支持AI摘要和原文保留两种内容处理模式，允许用户通过多种形式保存和查看市场信息。
...
`POST /api/market-fetcher/save` 接口 SHALL 支持 `contentMode` 参数和原文保存功能。
...
### Requirement: 市场信息列表展示

资产市场信息列表页面 SHALL 提供最新市场信息和时间范围查询两种视图。

#### Scenario: 最新消息视图

- **GIVEN** 用户访问市场信息列表页
- **WHEN** 选择"最新消息"标签
- **THEN** 系统 SHALL 显示最新的市场信息列表
- **AND** 每条记录显示标题、发布时间、情感标签、重要性标签
- **AND** 标题 SHALL 可点击跳转到详情页
...
- **WHEN** 选择"时间范围"标签
- **AND** 选择开始日期和结束日期
- **THEN** 系统 SHALL 显示该时间范围内的市场信息

#### Scenario: 关联资产标签展示

- **GIVEN** 市场信息列表展示多条记录
- **THEN** 每条记录 SHALL 显示关联的资产标签
- **AND** 点击标签 SHALL 跳转到对应资产详情页

### Requirement: 市场信息详情页

系统 SHALL 提供单个市场信息的详情展示页面。

#### Scenario: 详情页内容展示

- **GIVEN** 用户访问 `/asset-market-info/detail/[id]` 页面
- **THEN** 系统 SHALL 展示该市场信息的完整内容：
  - 标题、情感标签、重要性标签、发布时间
  - 关联资产列表（可点击查看资产详情）
  - 摘要（AI摘要模式）或原文内容（原文保留模式）
  - 市场影响分析
  - 关键话题（如有）
  - 关键数据点（如有）
  - 信息来源（来源名称和链接）
...
- **GIVEN** 用户在市场信息列表页
- **WHEN** 点击某条记录的标题
- **THEN** 系统 SHALL 跳转到该记录的详情页
- **AND** 详情页 URL 为 `/asset-market-info/detail/[id]`

#### Scenario: 详情页返回列表

- **GIVEN** 用户在市场信息详情页
- **WHEN** 点击"返回列表"按钮
- **THEN** 系统 SHALL 返回市场信息列表页
