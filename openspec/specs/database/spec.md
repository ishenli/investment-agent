...
## Purpose
管理系统数据库连接、迁移和数据持久化，支持 Web 和 Desktop (Electron) 环境。
...
每个数据表 MUST 包含标准字段：id、created_at、updated_at。

#### Scenario: 标准表字段
- **WHEN** 创建新的数据表
- **THEN** 表 MUST 包含自增的 `id` 字段
...
- **THEN** 系统 MUST 报错或无法访问
- **THEN** 数据库操作 MUST 通过 API Routes 进行